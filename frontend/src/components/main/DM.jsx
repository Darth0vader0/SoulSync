    ...
    if (messages.length > 0) {
      messages.forEach((msg) => {
        if (!msg.read && msg.receiverId === activeUser._id) {
          socket.emit("messageRead", { messageId: msg._id, senderId: msg.senderId, receiverId: selectedUser._id });
        }
      });
    }
  }, [messages, activeUser._id, selectedUser._id]);

  useEffect(() => {
    socket.on("messageReadUpdate", ({ messageId }) => {
      setMessages((prevMessages) =>
        prevMessages.map((msg) =>
          msg._id === messageId || msg.id? { ...msg, read: true } : msg
        )
      );
    });
